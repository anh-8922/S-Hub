...
export default function EditServiceAd() {
  const {id} = useParams()
  console.log("edit service provider id:", id)
  ...
  const [qulifications, setQulification] = useState ('')
  ...
  useEffect(() => {
    if (data && data.selectedServiceProviderAd
        ) {
      const {            
            category,
            subject,
            ...
            telephone,
            description,
            featured} = data.selectedServiceProviderAd
            setCategory(category)
            setLocation(location)
            setRate(rate)
            setExperience(experience)
            setQulification(qulifications)
